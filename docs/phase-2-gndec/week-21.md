# Week 21 — Applicant Tracking, Excel Export, and Recruiter Module
...
- Continued the Portal build by shifting focus to the student-facing side of drive management and the supporting modules for applicant tracking, Excel export, and the Recruiter dashboard.
...
- Observed that splitting the supply-side work (drives, rounds) and the demand-side work (student applications, applicant tracking, export, recruiter view) into separate weeks made each module's testing far more focused, because each week's deliverables could be validated against a clear, self-contained set of acceptance criteria.
